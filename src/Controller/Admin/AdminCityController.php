...
class AdminCityController extends AbstractController
{
     ...
     * @return Response
     */
    public function delete(Request $request, City $city, CommentRepository $commentRepository): Response
    {
        if ($this->isCsrfTokenValid('delete' . $city->getId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();

            foreach ($city->getPicture() as $picture) {
                $comments = $commentRepository->findBy(['picture' =>$picture->getId()]);
